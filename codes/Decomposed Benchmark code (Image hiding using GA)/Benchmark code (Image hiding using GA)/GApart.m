rng(seed)
options = optimoptions('ga','PlotFcns', @gaplotbestf,'Generations',10,...
    'PopulationSize',25);%,'StallGenLimit',inf);
nvar=7;
lowerBounds=[0 0 0 1 0 0 0];
higherBounds=[15 511 511 15 1 1 1];
f = @(chrom)ObjectiveFunc(chrom,forbiddenAreas,oldChromPixels);
[chrom,bestVal,flag] = ga(f,nvar,[],[],[],[],lowerBounds,higherBounds,[],1:nvar,...
    options);
% Chromosome parts
direction=chrom(1);
xOff=chrom(2);
...
forbiddenArea = messPixelSeq;
oldPixelsSequence = chromPixelSeq;